      ...
      success : function(r) {
        ...
        switch(r.error) {
          ...
          case 'not_found' :
            ...
            $('.alert').removeClass('hidden');
            $("form#register input[name=email]").val($("form#register input[name=email]").val());
            $("form#register input[name=email]").focus();
            break;
          case 'incorrect' :
...
function processSignup() {
  $("#register").submit(function() {
    var data = {};
    var isEmpty = 0;
    $("#register input").each(function(k,v) {
      if(!$(v).val().length) {
        $('.alert span').html('Please enter <strong>' + $(v).attr('name') + '</strong> !');
        $('.alert').removeClass('hidden');
        isEmpty++;
    ...
    if($("#register input[name=password]").val() != $("#register input[name=password2]").val()) {
      ...
      success : function(r) {
        ...
        switch(r.error) {
          ...
          case 'exists' : 
            ...
            $('.alert').removeClass('hidden');
            $("form#login input[name=email]").val($("form#register input[name=email]").val());
